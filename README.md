...
After you've included imgix.js on your page, it will automatically run once, after the `DOMContentLoaded` event fires. This will detect and process all `img` and `source` tags on the page that set up to use imgix.js as described in the [Usage](#usage) section of this readme.

imgix.js has a few global options:

* `host`: Your imgix hostname, has no default value. This enables the use of `ix-path` and `ix-params` to define images, instead of having to manually type URLs out in `imgix-src`. This has several advantages:
...
* `useHttps`: A boolean (default `true`), specifying whether to generate `http` or `https`-prefixed URLs.

These options can be defined in two ways. The first is setting them manually on the global `imgix.config` object:
...
The other option for setting global configuration options is specifying them with `meta` tags in your document's `head`. If present, these values will be detected just before the initial `DOMContentLoaded` call to `imgix.init()`:
...
If you need to display images from multiple imgix Sources, the `host` option can be overridden on any `img` or `source` tag by specifying an `ix-host` attribute:
